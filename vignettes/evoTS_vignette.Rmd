...
<br /> 
```{r eval = FALSE}
> fit.all.univariate(ln.diameter)
...
The Bokma model has a much better fit relative to the model where stasis in interrupted by a punctuation event. Where the jump happens can be investigated by plotting the data and putting a line at the estimated switch point, i.e. the first sample/population after the punctuation event.
