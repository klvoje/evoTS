#' @title Fit three models (Stasis, General Random Walk (trend), Stasis) to three segments of an evolutionary sequence (time-series).
#'
...
#'fit.Stasis.trend.Stasis(x, shift.point = c(20,40))
#'
fit.Stasis.trend.Stasis<-function (y, minb = 7, pool = TRUE, silent = FALSE, hess = FALSE, shift.point=NULL)
{
  ns <- length(y$mm)
  ng <- 3
  ...
  for (i in 1:nc) {
    ...
    gg <- shift2gg(GG[, i], ns)
        w <- opt.joint.Stasis.trend.Stasis(y, gg, pool = pool, hess = hess)
    logl[i] <- w$logL
    wl[[i]] <- w
  }
