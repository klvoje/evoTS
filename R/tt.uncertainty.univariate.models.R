...
#'y<-paleoTS:::sim.GRW(20,0.02,0.07)
#'
#'##Evaluate the effect of uncertainty in the timing of samples.
#'tt.uncertainty.univariate.models(y, model=c("URW", "GRW"), iter = 10)
#'

tt.uncertainty.univariate.models<-function(y, mode = "uniform", time.min = NULL, time.max = NULL, sd.time = NULL, model = c("URW","GRW"), pool = TRUE, iter=5){
  ...
  # Creating a new time vector based on assumtions about the error
    for (i in 1:iter){
      ...
      if ("Stasis" %in% model) {result.list$result.list_Stasis[[i]]<-paleoTS::opt.joint.Stasis(y_new_tt, pool = pool)
      }
      if ("URW" %in% model) {result.list$result.list_URW[[i]]<-paleoTS::opt.joint.URW(y_new_tt, pool = pool)
      }
  ...
  if ("Stasis" %in% model) {Stasis_aicc_values <- sapply(result.list$result.list_Stasis, function(x) {x$AICc})} else {Stasis_aicc_values<-NULL; } 
  if ("Stasis" %in% model) {Stasis_logl_values <- sapply(result.list$result.list_Stasis, function(x) {x$logL});
  Stasis_par_values <- sapply(result.list$result.list_Stasis, function(x) {x$parameters});
  Stasis.mean.par<-apply(Stasis_par_values,1,mean); Stasis.SD.par<-apply(Stasis_par_values,1,stats::sd) 
  }
  
  if ("URW" %in% model) {URW_aicc_values <- sapply(result.list$result.list_URW, function(x) {x$AICc})} else {URW_aicc_values<-NULL; } 
  if ("URW" %in% model) {URW_logl_values <- sapply(result.list$result.list_URW, function(x) {x$logL});
  URW_par_values <- sapply(result.list$result.list_URW, function(x) {x$parameters});
  URW.mean.par<-apply(URW_par_values,1,mean); URW.SD.par<-apply(URW_par_values,1,stats::sd) 
  }
  ...
  GRW.mean.par<-apply(GRW_par_values,1,mean); GRW.SD.par<-apply(GRW_par_values,1,stats::sd)
  }
  ...
  OU.fixed.mean.par<-apply(OU.fixed_par_values,1,mean); OU.fixed.SD.par<-apply(OU.fixed_par_values,1,stats::sd)
  }
  ...
  OUBM.mean.par<-apply(OUBM_par_values,1,mean); OUBM.SD.par<-apply(OUBM_par_values,1,stats::sd)
  }
  ...
  accel.mean.par<-apply(accel_par_values,1,mean); accel.SD.par<-apply(accel_par_values,1,stats::sd)
  }
  ...
  decel.mean.par<-apply(decel_par_values,1,mean); decel.SD.par<-apply(decel_par_values,1,stats::sd)
  }
