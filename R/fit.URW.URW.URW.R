#' @title Fit Unbiased Random Walk models to three different segments of an evolutionary sequence (time-series).
#'
#' @description Function to find maximum likelihood solutions to Unbiased Random Walk models to three different segments of an evolutionary sequence.
#'
...
#'fit.URW.URW.URW(x, shift.point = c(20,40))
#'

fit.URW.URW.URW<-function (y, minb = 7, pool = TRUE, silent = FALSE, hess = FALSE, shift.point=NULL)
{
  ns <- length(y$mm)
  ng <- 3
  ...
  for (i in 1:nc) {
    ...
    gg <- shift2gg(GG[, i], ns)
        w <- opt.joint.URW.URW.URW(y, gg, pool = pool, hess = hess)
    logl[i] <- w$logL
    wl[[i]] <- w
  }
