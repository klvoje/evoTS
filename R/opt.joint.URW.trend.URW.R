#' @title Fit thee models (Unbiased Random Walk, General Random Walk (trend), Unbiased Random Walk) to three segments of an evolutionary sequence (time-series).
#'
#' @description Function to find maximum likelihood solutions to three models to three different segments of an evolutionary sequence. Most users will interact with this function through the function fit.URW.trend.URW.
#'
...
opt.joint.URW.trend.URW<-function (y, gg, pool = TRUE, meth = "L-BFGS-B", hess = FALSE) 
{
  if (pool) {
  ...
  small <- 1e-08
    ...
    p0rw_2<- paleoTS::mle.URW(paleoTS::sub.paleoTS(y, ok = gg == 3))
