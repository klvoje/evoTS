#' @title Fit thee models (Unbiased Random Walk, General Random Walk (trend), Stasis) to three segments of an evolutionary sequence (time-series).
#'
#' @description Function to find maximum likelihood solutions to three models to three different segments of an evolutionary sequence. Most users will interact with this function through the function fit.URW.trend.Stasis.
#'
...
opt.joint.URW.trend.Stasis<-function (y, gg, pool = TRUE, meth = "L-BFGS-B", hess = FALSE) 
{
  if (pool) {
  ...
  small <- 1e-08
    ...
    p0st<- paleoTS::mle.Stasis(paleoTS::sub.paleoTS(y, ok = gg == 3))
  ...
  if (meth != "L-BFGS-B") 
    ...
    w <- try(optim(p0, fn = logL.joint.URW.trend.Stasis, gg = gg, 
  ...
  if (class(w) == "try-error") {
    wc <- paleoTS::as.paleoTSfit(logL = NA, parameters = NA, modelName = paste("URW", 
                                                                      "Trend","Stasis", sep = "-"), method = "Joint", K = K, n = length(y$mm), 
                        se = NULL)
    return(wc)
  }
  if (hess) 
