#' @title Fit thee models (Unbiased Random Walk, Unbiased Random Walk, Unbiased Random Walk) to three segments of an evolutionary sequence (time-series).
#'
#' @description Function to find maximum likelihood solutions to three models to three different segments of an evolutionary sequence. Most users will interact with this function through the function fit.URW.URW.URW.
#'
...
opt.joint.URW.URW.URW<-function (y, gg, pool = TRUE, meth = "L-BFGS-B", hess = FALSE) 
{
  if (pool) {
  ...
  small <- 1e-08
    ...
    p0rw_3 <- paleoTS::mle.URW(paleoTS::sub.paleoTS(y, ok = gg == 3))
  ...
  if (p0rw_3["vstep"] <= small) 
    p0rw_3["vstep"] <- 100 * small
  ...
  p0 <- c(p0anc, p0rw_1, p0rw_2, p0rw_3)
  ...
  if (meth != "L-BFGS-B") 
    ...
    w <- try(optim(p0, fn = logL.joint.URW.URW.URW, gg = gg, 
  ...
  if (class(w) == "try-error") {
    wc <- paleoTS::as.paleoTSfit(logL = NA, parameters = NA, modelName = paste("URW", 
                                                                      "URW", "URW", sep = "-"), method = "Joint", K = K, n = length(y$mm), 
                        se = NULL)
    return(wc)
  }
  if (hess) 
