#' @title Fit thee models (Stasis, General Random Walk (trend), Stasis) to three segments of an evolutionary sequence (time-series).
#'
...
opt.joint.Stasis.trend.Stasis<-function (y, gg, pool = TRUE, meth = "L-BFGS-B", hess = FALSE) 
{
  if (pool) {
  ...
  small <- 1e-08
    ...
    p0st_2<- paleoTS::mle.Stasis(paleoTS::sub.paleoTS(y, ok = gg == 3))
  ...
  if (p0st_2["omega"] <= small) 
    p0st_2["omega"] <- 100 * small
  
    #make vector out of estimated model parameters
  p0 <- c(p0st_1, p0dt, p0st_2)
  ...
  if (meth != "L-BFGS-B") 
    ...
    w <- try(optim(p0, fn = logL.joint.Stasis.trend.Stasis, gg = gg, 
