#' @title Fit two Unbiased Random Walk models separated by a single punctuation (Bokma model) to an evolutionary sequence (time-series).
#'
#' @description Function to find maximum likelihood solutions to the Bokma model with a single punctuation to an evolutionary sequence. Most users will interact with this function through the function fit.Bokma.
#'
...
opt.joint.Bokma<-function (y, gg, pool = TRUE, hess = FALSE, meth="L-BFGS-B")
{
  if (pool) {
  ...
  names(p0) <- c("anc", "vstep_1", "new.opt", "vstep_2")
  K <- 5

  cl$ndeps <- p0/100

      w <- optim(p0, fn = logL.joint.Bokma, gg = gg, method = meth,
  ...
  wc <- paleoTS::as.paleoTSfit(logL = w$value, parameters = w$par, modelName = paste("Bokma model",
